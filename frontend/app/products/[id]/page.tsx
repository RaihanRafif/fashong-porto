...
import { notFound } from 'next/navigation'; // Impor fungsi notFound

// Kita akan pindahkan tipe data ini ke file terpisah nanti untuk praktik terbaik,
// tapi untuk sekarang tidak apa-apa di sini.
type Product = {
  id: string;
  name: string;
  price: string;
...
export default async function ProductDetailPage({ params }: { params: { id: string } }) {
  // --- SOLUSI: Logika fetch data dipindahkan ke sini ---
  let product: Product | null = null;
  try {
    const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/products/${params.id}`;
    ...
    if (res.status === 404) {
      ...
      // Ini akan menampilkan halaman 404 bawaan Next.js yang sudah bagus.
      notFound();
    }
  ...
  // Jika karena alasan lain produk tetap null (seharusnya tidak terjadi jika notFound() bekerja)
  if (!product) {
    notFound();
  }
      ...
      <div className="container mx-auto p-8">
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div>
            ...
            {/* Nanti tombol AI Stylist akan kita letakkan di sini */}
          </div>
        </div>
      </div>
